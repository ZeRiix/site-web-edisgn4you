...
if(isset($_GET['type']) AND $_GET['type'] == 'annonces') {
   if(isset($_GET['supprime']) AND !empty($_GET['supprime'])) {
      ...
      $req = $bdd->prepare('DELETE FROM annonces WHERE id = ?');
      $req->execute(array($supprime));
   }
}
$annonces = $bdd->query('SELECT * FROM annonces ORDER BY id DESC LIMIT 0,5');
$annoncesList = $bdd->query('SELECT * FROM annonces ORDER BY id');
?>
