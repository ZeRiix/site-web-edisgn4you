                     		...
                     		if($mailexist === 0) {
                        		...
                        		if($mdp == $mdp2) {
									...
									for ($i = 1; $i < $sizeKey; $i++) {
                           			...
                           			$insertMember = $bdd->prepare("INSERT INTO membres(pseudo,mail,motdepasse,prenom,nom,date_crea,pays,num, descriptions) VALUES(?,?,?,?,?,?,?,?,?)");
                           			$insertMember->execute(array($pseudo, $mail, $mdp, $name, $surname, $date, $country, $num, $description));

                           			header('location: connexion.php?erreur=Votre compte a bien été créé !');
										   ...
										   <html>
												  <body>
													 <div align="center">
														...
														<a href="https://e-design4you.com/confirme.php?mail=.urlencode($mail).?key=.$key</a>
													 </div>
												  </body>
										   </html>
										  ';
                           			...
                           			header('location: inscription.php?erreur=Vos mots de passes ne correspondent pas !');
                        		}
                     		} else {
                        		header('location: inscription.php?erreur=Adresse mail déjà utilisée !');
                     		}
                  		} else {
                     		header('location: inscription.php?erreur=Votre adresse mail n\'est pas valide !');
                  		}
               		} else {
                  		header('location: inscription.php?erreur=erreur dans la structuture du num tel');
               		}
            	} else {
               		header('location: inscription.php?erreur=erreur dans la structuture de votre nom');
            	}
        	} else {
        		header('location: inscription.php?erreur=erreur dans la structuture du notre prénom');
        	}
    	} else {
        	if ($namelenght > 60) {
            	header('location: inscription.php?erreur=votre pseudo est trop long');
        	}
        	if ($namelenght < 7) {
            	header('location: inscription.php?erreur=votre pseudo est trop court');
        	}
    		}
   	} else {
      	header('location: inscription.php?erreur=Tous les champs doivent être complétés !');
   	}
}
?>
<html>

	<?php $namePage = 'Inscription'; include('includes/head.php'); ?>
        ...
        <div class="spacer50" id="spacer50"></div>
          <div class="container-fluid">
            <div class="container" id="container-inscription">
              ...
              <h2>Vous êtes un vendeur professionnel ? <a href="inscriptionPro.php">Créer un compte professionnel<a></h2>
